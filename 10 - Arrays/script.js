...
dogs.forEach((elm, indx, arr) => {
  elm.reccommendedFood = Math.trunc(elm.weight ** 0.75 * 28);
});
console.log(dogs);

const sarahDog = dogs.find(el => el.owners.includes('Sarah'));
console.log(calcPortion(sarahDog));
showHealthy(sarahDog);
console.log(sarahDog);

function calcPortion(dog) {
  if (dog.curFood > dog.reccommendedFood) return "it's eating too much";
  else if (dog.curFood < dog.reccommendedFood) return "it's eating too little";
  else return 'It is Ok.';
}

function showHealthy(dog) {
  dog.isHealthy =
...
dogs.forEach((elm, indx, arr) => {
  showHealthy(elm);
  elm.curFood > elm.reccommendedFood
    ? ownersEatTooMuch.push(elm)
    : ownersEatTooLittle.push(elm);
});

console.log(ownersEatTooLittle);
console.log(ownersEatTooMuch);

const printLogs = dogs => {
  const owners = dogs.flatMap(el => el.owners).join(' and ');
  return `${owners} dogs eat too ${dogs.isHealthy ? 'much' : 'little '}`;
};

console.log(printLogs(ownersEatTooLittle));
console.log(printLogs(ownersEatTooMuch));

const anyHealthyDog = dogs.some(el => el.reccommendedFood === el.curFood);
console.log(anyHealthyDog);

const isHgealthyDog = dogs.some(el => el.isHealthy === true);

console.log(
  `${isHgealthyDog}`
    ? 'There is dog that eats exact amount'
    : `There is not dog that eats exact amount`
);

const healthyDogArr = dogs.filter(
  el =>
    el.curFood > el.reccommendedFood * 0.9 &&
    el.curFood < el.reccommendedFood * 1.1
);

console.log(healthyDogArr);

const sortArr = dogs
  .slice()
  .map(el => el.reccommendedFood)
  .sort((cur, nex) => cur - nex);

console.log(sortArr);

const sortObject = dogs
  .slice()
  .sort((a, b) => a.reccommendedFood - b.reccommendedFood);
