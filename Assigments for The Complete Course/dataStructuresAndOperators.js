...
const normalizeAuthorName = function (authorInfo) {
  ...
  authorInfo = authorInfo.replace('(contributor)', ' ');

  const authorName =
    authorInfo[0].toUpperCase() + authorInfo.slice(1, authorInfo.indexOf(' '));

  let authorLastname = authorInfo.slice(authorInfo.indexOf(' ') + 1);
  authorLastname = authorLastname[0].toUpperCase() + authorLastname.slice(1);
...
const res = quote.slice(startIndex, startIndex + 7);
const res2 = quote.slice(quote.lastIndexOf(' ') + 1);
quote = quote.slice(0, startIndex) + quote.slice(startIndex + 6);
console.log(quote);
console.log(res);
console.log(res2);
*/
...
const isContributor = function (authorName) {
  ...
  return authorName.indexOf('(Contributor)') !== -1;
};
...
for (const [i, element] of Object.values(
...
const getFirstKeyword = function (bookArr) {
  ...
  for (const item of bookArr) {
    console.log(item.keywords?.[0] ?? "Property doesn't exist. ");
  }
};
...
console.log(bookData[0][0], typeof bookData[0][0], [bookData[0][0]]);
const newBook = {
...
const spellWord = function (arg) {
  const a = [...arg];
  const b = 'J a v a S c r i p t';
  const c = ['J', 'a', 'v', 'a', 'S', 'c', 'r', 'i', 'p', 't'];
